...
export const AddPrescriptionInfo = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch patient
        const patientResponse = await fetch(`https://localhost:44320/api/Patients/${id}`);
        if (!patientResponse.ok) throw new Error("Failed to fetch patient");
        const patientData = await patientResponse.json();
        setPatient(patientData);

        // Fetch doctors
        const doctorsResponse = await fetch('https://localhost:44320/api/Doctors');
        if (!doctorsResponse.ok) throw new Error("Failed to fetch doctors");
        const doctorsData = await doctorsResponse.json();
        setDoctors(doctorsData);

        // Fetch drugs
        const drugsResponse = await fetch('https://localhost:44320/api/Drugs');
  ...
  async function handleSave() {
    ...
    try {
      ...
      const response = await fetch('https://localhost:44320/api/Prescriptions', {
        method: 'POST',
        headers: {
